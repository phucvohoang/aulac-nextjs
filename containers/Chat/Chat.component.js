...
class Chat extends React.Component {
  ...
  handleSendChatImage = async (url) => {
    ...
    if (currentChat) {
      ...
      try {
        ...
        console.log(e);
        console.log(
          '========== Could not Upload Image to Firebase ================'
        );
      }
    }
  };
  ...
  chooseChatWith = async (friend, isFriend) => {
    ...
    if (isFriend) {
      ...
      const listMessages = await firestore
        .collection('messages1')
        .doc(`${groupId}`)
        .collection('conversations')
        .get();
      ...
      if (existingUser.size === 0) {
        await firestore.collection('friends').add({
          avatar:
            'https://aulac-media.s3.ap-southeast-1.amazonaws.com/images/avatars/5fc73e0eb3d7ea758151f5cf-1607964767871.jpg',
          deviceToken: '',
          email: friend.email,
          name: friend.name,
          uid: friend._id,
    ...
    if (listMessages.length > 0) {
      return listMessages.map((msg) => {
        ...
        return (
          <div
            className={`message ${
              msg.uid !== currentUser._id ? 'received' : 'sent'
            }`}
          >
      ...
      if (this.props.searchUsers.length > 0) {
        return this.props.searchUsers.map((friend) => {
          return (
            <li
              onClick={() => {
                this.chooseChatWith(friend, friend.uid);
              }}
              className={`chat-link-signle ${
                currentChat?.uid === friend.uid ? 'active' : ''
              }`}
            >
    ...
    if (listDriver.length > 0) {
      return listDriver.map((driver) => {
        return (
          <li
            onClick={() => {
              this.chooseDriverToChat(driver);
            }}
            className={`chat-link-signle ${
              currentChat?._id === driver._id && 'active'
            }`}
          >
            <div className="chat__content--user">
              <div className="author-figure">
      ...
      return (
        <li style={{ width: '100%', padding: '0rem 2rem' }}>
          Đơn hàng bạn chưa được nhận bởi tài xế
        </li>
      );
    }
  };
      ...
      <div className="chat__page">
        <div className="chat__page__left">
          ...
          <div className="chat__page__left--form">
            <p style={{ width: '100%', padding: '0rem 2rem' }}>
              Trạng Thái Đơn Hàng
            </p>
        ...
        <div className="chat__page__right">
          <div className="chat__page__right--title">
            <h2 className="name">
              {this.state.currentChat && this.state.currentChat.name}
            </h2>
